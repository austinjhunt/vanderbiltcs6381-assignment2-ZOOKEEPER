...
while getopts c:t:m:i:b:s:p: flag
do
    case "${flag}" in
        c) count=${OPTARG};;
        t) topics+=(${OPTARG});;
...
if [ -z "$max_event_count" ] ; then
    ...
    # Use max event count
    echo "python ../driver.py --subscriber $count $topic_arguments $producer_arguments --indefinite"
    python ../driver.py --subscriber $count $topic_arguments $producer_arguments --indefinite
else
    # Use max event count
    echo "python ../driver.py --subscriber $count $topic_arguments $producer_arguments --max_event_count $max_event_count"
    python ../driver.py --subscriber $count $topic_arguments $producer_arguments --max_event_count $max_event_count
fi
